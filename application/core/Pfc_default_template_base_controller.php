...
abstract class Pfc_default_template_base_controller extends Pfc_i18n_base_controller {
    ...
    function __construct ()
    {
        ...
        if (!$this->session_service->has_session()){
        ...
        $this->get_user_connected();
		$this->get_time_next_order();
    }
    ...
    private function get_user_connected(){
    	...
    	$data = $this->session_service->get_data_user_connected();
    	$id = safe_array_get('i_id_usuario',$data);
    	$data = $this->user_model->get_user_by_id($id);
    	$name_user = safe_array_get('s_nombre_completo',$data);
    	$role = safe_array_get('c_tipo_usuario',$data);
    	switch($role){
    		case "S":
    			$role = "SuperAdmin";
    			break;
    	...
    	$name_user = array("user_name"=>$name_user, "role"=>$role);
    	parent::add_template_datavars($name_user);
    	
    }
    
    private function get_time_next_order(){
    	$this->load->model('order_model','',TRUE);
    	
    	$group = 1;
    	
    	$data = $this->order_model->get_time_next_order($group);
    	$data = date("d/m/Y",strtotime(safe_array_get('dt_fecha_pedido',$data)));
    	$data = array("date_next_order"=>$data);
    	parent::add_template_datavars($data);
